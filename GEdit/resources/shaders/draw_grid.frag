...
*/
    ...
    if(t>0)
        mod = 0;  
    float rez = 1.0 * float(t > 0);
    if(gl_FragCoord.y > 0)
        outColor = vec4(farPoint, 1); // opacity = 1 when t > 0, opacity = 0 otherwise
    else
        outColor = vec4(nearPoint, 1); // opacity = 1 when t > 0, opacity = 0 otherwise
     */
